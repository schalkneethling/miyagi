...
export default () => {
  ...
  /**
   * When `--perf-report <path>` was passed and miyagi.performance.json
   * exists, run the performance check against the just-built tree and
   * write the markdown report. Never throws — perf is opt-in and a missing
   * config is treated as "feature disabled".
   * @returns {Promise<void>}
   */
  async function maybeWritePerfReport() {
    const reportPath = global.config.perfReport ?? global.config["perf-report"];
    if (!reportPath) {
      return;
    }
    try {
      ...
      if (!result.enabled) {
        log(
          "warn",
          "miyagi build --perf-report given but no miyagi.performance.json found.",
        );
        return;
      }
  ...
  async function buildUserAssets(buildFolder, assetsConfig, logoPaths) {
    ...
    const sharedCss = (assetsConfig.shared?.css || []).map((file) =>
      path.join(assetsConfig.root, typeof file === "string" ? file : file.src),
    );
