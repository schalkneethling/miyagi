...
describe("drupal-assets CLI integration", () => {
	beforeEach(async () => {
		await cp(path.join(FIXTURES, "components"), path.join(WORK_DIR, "components"), {
			recursive: true,
		});

		global.config = {
			components: { folder: path.join(WORK_DIR, "components") },
			files: { mocks: { name: "mocks", extension: ["yaml", "yml", "json", "js"] } },
		};
	});
	...
	test("updates all components when no --components specified", async () => {
		...
		expect(alertBox.$assets.css).toStrictEqual(["build/assets/css/alert-box.css"]);
	...
	test("ignores external dependencies based on ignorePrefixes", async () => {
		...
		expect(content.$assets.js).toStrictEqual([{ src: "build/assets/js/button.js" }]);
